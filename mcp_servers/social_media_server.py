...
if __name__ == "__main__":
    ...
    print(f"Facebook        : {'✓' if FB_PAGE_ACCESS_TOKEN and FB_PAGE_ID else '✗ (dry-run)'}")
    print(f"Instagram       : {'✓' if FB_PAGE_ACCESS_TOKEN and IG_USER_ID else '✗ (dry-run)'}")
    tw = all([TWITTER_API_KEY, TWITTER_API_SECRET, TWITTER_ACCESS_TOKEN, TWITTER_ACCESS_SECRET])
    print(f"Twitter/X       : {'✓' if tw else '✗ (dry-run)'}")
    print("=" * 55)
    mcp.run()
